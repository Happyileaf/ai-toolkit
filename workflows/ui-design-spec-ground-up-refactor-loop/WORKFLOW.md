...
3. 若设计规范提供模板，必须先判断是否存在符合当前页面设计目标的模板；若存在，优先基于该模板重构；若不存在，才按规范自主设计。
4. 每一页开始前，必须先产出页面蓝图：有匹配模板时为“Template-Driven 蓝图”，无匹配模板时为“Spec-Only 蓝图”。
5. 每轮必须声明“本轮引用的规范章节”和“本轮要落地的蓝图条目”。
6. 未通过规范对齐与功能等价验证，不得判定页面完成。
7. 若改动被判定为“补丁式微调”，必须触发下一轮“去锚点重做”而不是结束。
8. 全程仅允许 UI 层改动，不得改业务逻辑、数据流、接口契约。
...
2. 本轮引用规范章节（标题/编号/路径）
3. 当前页面蓝图模式（`template_driven` 或 `spec_only`），若为前者需注明模板引用
4. 本轮落地蓝图条目
5. 本轮计划改动点
6. 本轮实际改动文件清单
7. 验证结论：
