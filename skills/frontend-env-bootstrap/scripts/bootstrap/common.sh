...
feb_is_known_error_code() {
  feb_all_error_codes | grep -Fx "$1" >/dev/null 2>&1
}
...
feb_should_simulate_error() {
  local code="$1"
  local configured="${FEB_SIMULATE_ERROR_CODES:-${FEB_SIMULATE_ERROR_CODE:-}}"
...
feb_maybe_simulate_error() {
  local code="$1"
  local stage="$2"
  local message="$3"
  local next_action="$4"
  local retryable="${5:-false}"
  if feb_should_simulate_error "$code"; then
    feb_set_error "$code" "$message" "$stage" "$retryable" "$next_action" "simulated error via FEB_SIMULATE_ERROR_CODE(S)"
    return 1
  fi
  return 0
}
...
feb_validate_node_lts_policy() {
  case "$1" in
    latest_lts) return 0 ;;
    fixed:*)
      if printf '%s' "${1#fixed:}" | grep -E '^[0-9]+\.[0-9]+\.[0-9]+$' >/dev/null 2>&1; then
        return 0
      fi
      return 1
      ;;
...
feb_write_failure_verification_report_placeholder() {
  local reason="$1"
  cat >"$FEB_VERIFICATION_REPORT" <<EOF
{
...
feb_ensure_verification_report_exists() {
  if [ ! -f "$FEB_VERIFICATION_REPORT" ]; then
    feb_write_failure_verification_report_placeholder "verification_report.json missing; placeholder generated by bootstrap finalize."
  fi
}
...
{
  "platform": "$(feb_json_escape "$FEB_PLATFORM")",
  "target_platforms": "$(feb_json_escape "${FEB_TARGET_PLATFORMS:-$FEB_PLATFORM}")",
  "shell_preference": "$(feb_json_escape "${FEB_SHELL_PREFERENCE:-unknown}")",
  "dry_run": $FEB_DRY_RUN,
