#!/usr/bin/env python3
"""
frontend-harness scaffold generator.
...
MANAGED_BLOCK_BEGIN = "<!-- BEGIN FRONTEND-HARNESS MANAGED BLOCK -->"
MANAGED_BLOCK_END = "<!-- END FRONTEND-HARNESS MANAGED BLOCK -->"
AGENT_DOC_ALIASES = {
    "AGENTS.md": ("AGENTS.md", "AGENT.md", "agents.md", "agent.md"),
    "CLAUDE.md": ("CLAUDE.md", "claude.md"),
}
FRONTEND_RULE_FILES = (
    "docs/frontend/rules/00-core.md",
    "docs/frontend/rules/state-and-dataflow.md",
    "docs/frontend/rules/ui-ux-a11y.md",
    "docs/frontend/rules/network-and-security.md",
    "docs/frontend/rules/testing-and-release.md",
    "docs/frontend/rules/performance-and-observability.md",
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Initialize frontend-harness standalone scaffold."
    )
...
def looks_like_legacy_frontend_harness_doc(content: str) -> bool:
...
def merge_markdown_block(existing: str, managed_content: str) -> str:
    ...
    if looks_like_legacy_frontend_harness_doc(existing):
        return block
...
def find_existing_agent_doc(target_path: Path) -> Optional[Path]:
...
def verify_outputs(target_dir: Path):
    required = [
        ...
        *[f"{HARNESS_DIR_NAME}/{rel}" for rel in FRONTEND_RULE_FILES],
...
def main():
    ...
    file_mappings = [
        ...
        *[
            ...
            for rel in FRONTEND_RULE_FILES
        ],
    ...
    print(f"\nInitializing frontend-harness in: {target_dir}")
