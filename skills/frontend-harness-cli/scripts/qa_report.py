#!/usr/bin/env python3
"""
Build frontend-harness QA reports from contract matrices and QA command outputs.

The script is intentionally dependency-free so initialized projects can run it in
plain frontend repositories without extra Python packages.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate a frontend-harness QA report.")
        ...
        matrix.append(
            {
                ...
                "scenario": value_by_alias(row, ("场景/页面", "场景", "Scenario")),
                "test_class": value_by_alias(row, ("测试入口", "测试类", "Test Class")),
...
def attach_matrix_evidence(matrix: list[dict[str, str]], commands: list[dict[str, object]]) -> list[dict[str, str]]:
    e2e_status = command_status(commands, "frontend_e2e")
    e2e_command = next((cmd for cmd in commands if cmd.get("name") == "frontend_e2e"), {})
    e2e_summary = str(e2e_command.get("summary", ""))
    for item in matrix:
        ...
        if e2e_status == STATUS_PASS:
            ...
            item["evidence"] = f"frontend_e2e: PASS ({e2e_summary})"
            continue
        if e2e_status == STATUS_FAIL:
            item["status"] = STATUS_FAIL
            item["evidence"] = f"frontend_e2e: FAIL ({e2e_summary})"
            continue
        if test_class:
            item["status"] = STATUS_PARTIAL
            item["evidence"] = f"frontend_e2e skipped; scenario hint={test_class}"
        else:
            item["status"] = STATUS_FAIL if gate == GATE_REQUIRED else STATUS_PARTIAL
            item["evidence"] = "frontend_e2e skipped and no scenario hint"
    return matrix
...
def compute_gate_status(matrix: list[dict[str, str]], commands: list[dict[str, object]]) -> tuple[str, dict[str, int]]:
    ...
    blocking_failures = []
    for name in ("convention_check", "frontend_lint", "frontend_typecheck", "frontend_unit", "frontend_build"):
    ...
    if required and (summary["required_failed"] or summary["required_passed"] < summary["required_total"]):
...
def mark_criteria(criteria: list[dict[str, str]], commands: list[dict[str, object]]) -> list[dict[str, str]]:
    unit_status = command_status(commands, "frontend_unit")
    build_status = command_status(commands, "frontend_build")
    e2e_status = command_status(commands, "frontend_e2e")
    lint_status = command_status(commands, "frontend_lint")
    typecheck_status = command_status(commands, "frontend_typecheck")
    for item in criteria:
        method = str(item.get("verification_method", "")).lower()
        if "integration" in method or "集成" in method or "e2e" in method:
            item["status"] = e2e_status if e2e_status in {STATUS_PASS, STATUS_FAIL} else STATUS_PARTIAL
        elif "typecheck" in method or "类型" in method:
            item["status"] = typecheck_status if typecheck_status in {STATUS_PASS, STATUS_FAIL} else STATUS_PARTIAL
        elif "lint" in method:
            item["status"] = lint_status if lint_status in {STATUS_PASS, STATUS_FAIL} else STATUS_PARTIAL
        elif "build" in method:
            item["status"] = build_status if build_status in {STATUS_PASS, STATUS_FAIL} else STATUS_PARTIAL
        elif "unit" in method or "单元" in method:
            item["status"] = unit_status if unit_status in {STATUS_PASS, STATUS_FAIL} else STATUS_PARTIAL
...
def render_markdown(result: dict[str, object]) -> str:
    ...
    lines.extend(["", "### 集成测试矩阵", "", "| ID | 验收项 | 依赖 | 场景/页面 | 测试入口 | 门禁 | 状态 | 证据 |", "| --- | --- | --- | --- | --- | --- | --- | --- |"])
    for item in result.get("integration_matrix", []) or []:
        lines.append(
            "| {id} | {criterion} | {dependency} | {scenario} | {test_class} | {gate} | {status} | {evidence} |".format(
                id=item.get("id", ""),
                criterion=str(item.get("criterion", "")).replace("|", "/"),
...
def build_result(
    ...
    matrix = attach_matrix_evidence(matrix, commands)
    ...
    return {
        ...
        "reports": {
            "frontend_commands": [command.get("name", "") for command in commands],
        },
    }
