...
CHECKLIST = [
    ...
    "6. 行为变化时，相关文档是否已更新？",
    "7. lint/typecheck/unit/build/e2e（按 contract 要求）是否完成并记录结果？",
    "8. 响应式（桌面+移动）与关键交互流是否已验证？",
    "9. 无障碍关键项（键盘可达、语义标签、图片 alt）是否已自检？",
    "10. 设计 token/CSS 变量是否复用，避免散落魔法值？",
    "11. convention-check.py --changed-only 是否无 FAIL，WARN 是否已修复或解释？",
]
