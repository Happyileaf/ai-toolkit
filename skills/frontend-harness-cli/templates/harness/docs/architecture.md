...
| QA Runner | `.harness/scripts/qa_runner.py` | 执行 frontend_doctor、convention-check、lint/typecheck/unit/build/e2e、Agent Review 并生成 QA Gate 结果 |
| Agent Review | `.harness/scripts/agent_review.py` | 统一 Codex/Claude 代码审查收口，归一化 accepted/actionable finding |
| QA Report | `.harness/scripts/qa_report.py` | 绑定 contract 验收标准与场景矩阵，汇总命令结果与门禁状态 |
| QA Gate | `.harness/scripts/qa_gate.py` | 根据 result JSON 判断 required 门禁是否通过 |
| Frontend Doctor | `.harness/scripts/testcontainers_doctor.py` | 检查 Node、包管理器与 package scripts 可用性 |
...
| MCP 工具 | 使用方 | 目的 |
|----------|---------|---------|
| Node / pnpm / yarn / npm / bun | Evaluator / QA Runner | lint/typecheck/unit/build/e2e 验证 |
| Playwright / Cypress（按项目） | Evaluator / QA Runner | 前端关键路径场景验证 |
| convention-check | Evaluator | 前端约定机器检查 |
...
状态：COMPLETE（文档更新，冲刺日志记录，必要时更新 passes）
  |
  [lint/typecheck/unit/build/convention-check 或 required 场景测试失败 且迭代 < 3]
  v
