...
Evaluator 使用前端工程验证策略：
1. **单元层**：读源码校验逻辑，并运行可用单元测试
2. **场景层**：读取 contract 集成测试矩阵，运行 e2e/手测场景，覆盖关键路径、异常路径与跨页面流程
3. **契约层**：逐条对照 contract、前端总门禁和触发维度分片规则
...
**执行门禁**：lint/typecheck/unit/build 通过、`convention-check` 无 fail、contract 中 required 场景测试通过，且 required Agent Review（如启用）无 accepted/actionable finding

**阶段流转条件**：
- required QA Gate 与 required Agent Review（如启用）通过：进入阶段 6（Complete）
- lint/typecheck/unit/build、convention-check、required 场景测试或 required Agent Review 失败：进入阶段 5（Fix），一次性修复所有 Agent Review accepted finding 和测试失败，不再重新审查
...
- lint/typecheck/unit/build 通过、required QA Gate 通过、required Agent Review（如启用）通过，且 `spec_path` / `contract_path` 文件真实存在后，才可更新对应 feature 的 `passes=true`
