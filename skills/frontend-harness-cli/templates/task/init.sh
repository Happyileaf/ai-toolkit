...
if [ -f "$REPO_ROOT/AGENTS.md" ]; then
  ...
  echo "  ⚠️ AGENTS.md 不存在，建议先执行 frontend-harness 脚手架初始化"
fi
...
if [ -f "$TASK_CONTRACT_FILE" ]; then
  ...
  echo "  ⚠️ TASK-HARNESS.md 不存在，建议重新执行 frontend-harness 脚手架"
fi
...
echo "  5. 按 plan/build/qa gate/agent review 流程执行，required 门禁通过后才可改 passes"
echo "  6. QA 建议命令：python3 ${WORKSPACE_PREFIX}scripts/qa_runner.py --target-dir . --contract '<contract 文件>'"
